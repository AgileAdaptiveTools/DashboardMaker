...
Ext.define('changingChild', {
    ...
    unselect: function () {
    	if(this.active){
			...
			if(numberOfSources != selectedSources){
    			...
    			console.log("NO LONGER READY! DISABLING :( ", b);
    			b.disabled = "disabled";
    		}
			selectedSources--;
			console.log("Going DOWN", selectedSources);
	...
	select: function () {
		...
		selectedSources++;
    	console.log("Going UP", selectedSources);

    	if(numberOfSources == selectedSources){
    		var b = document.getElementById("confirm_button_html");
    		console.log("READY! ENABLING: ", b);
    		b.disabled = false;
    	}
    	
    	window["widgetSelected"+this.sourceNumber] = {"name":this.universalName, "data":this.data};
    	console.log("widgetSelected"+this.sourceNumber, " set to: ", window["widgetSelected"+this.sourceNumber]);
...
for (i in test_array){
	console.log(arrayMinusIndex(test_array, i));
}
*/
...
function generateChangingWidgetIcon(sourceNumber, indexNumber, widget, main_rule, sub_rule){
	...
	console.log("Generating widgetChoice with id: ", "Widget-"+sourceNumber+"-"+indexNumber);
	var returnImg = Ext.create('changingIcon', {
		...
		listeners: {
			'afterrender': function(){
				...
				console.log("getPosition = ", this.getPosition(true));
				console.log("getPosition = ", this.getPosition(false));
				console.log("X = ", this.getEl().getXY());
				...
				*/
				...
				for(i in this.color_child_images){
					//console.log("generating child with id: ", id+"-"+i);
					//console.log("position will be: ", this.x+(70*i)+5, this.y+120);
					if (i == 0) {
					...
					//console.log("new_child0 = ", new_child0);
				...
				//console.log("Children set to: ", children);
			...
			'added': function(){
				...
				//console.log("uid + widget.id = ", uid + widget.id);
			...
			el: {
				click: function(eventObject, element) {
					...
					//console.log(parent);
					//console.log("Siblings are: ", parent.siblings);
					for (i in parent.siblings){
						//console.log("i is: ", i);
						//console.log("Shutting down: ", parent.siblings[i]);
						parent.siblings[i].unselect();	
					} 
					...
					if(!parent.displaying_children){
						...
						//console.log("clicked on icon: ", parent.id);
						//console.log("Parent.children are: ", parent.children);
						for(i in parent.children){
							//console.log("Now displaying: ", parent.children[i].id);
				...
				mouseenter: function(eventObject, element){
					...
					//console.log("id is either: ", this.id, ", or ", element.id);
    ...
    //console.log("returnImg's id is: ", returnImg.id);
    return returnImg;
};
...
//	id: "TEMPORARY",
	...
	*/
	...
	//console.log("Defining widgetPanel with widgetPanels: ", widgetPanels);
	...
	*/
	//console.log("Returning widget panel: ", widgetPanel);
	return layoutPanel;
}
...
//console.log("example_panel = ", example_panel);
...
Ext.define('Ext.ux.Image', {
    ...
    onRender: function() {
    	//console.log("this.initialConfig: ", this.initialConfig);
...
Ext.define('Widget_Item', {
	...
	constructor: function(name, src, child_images) {
		console.log("Constructor called with: ", name, ", ", src, ", ", child_images);
		//this.initComponent();
		this.name = name;
		...
		this.displaying_children = false;
		console.log("Preparing to generateItems: ", this.items);
		this.items = this.generateItems();
		console.log("Finished preparing items: ", this.items);
		return this;
	},
	
	generateItems: function(){
		console.log("GenerateItems will return: ", this.generateImgFromSrc(this.src));
...
Ext.define('Widget_Item_Old', {
	...
	constructor: function(name, src, child_images) {
		console.log("Constructor called with: ", name, ", ", image_source, ", ", child_images);
		this.initComponent();
		this.name = name;
		this.src = src;
		this.child_images = child_images;
	...
	setSrc: function(src) {
        ...
        if (this.rendered) {
        	console.log("Rendered: Using DOM");
            this.el.dom.src = src;
            this.src = src;
        } else {
        	console.log("Not rendered: setting widget property");
            this.src = src;
        }
    },

    getSrc: function(src) {
    	console.log("getSrc returning: ", this.el.dom.src || this.src);
        return this.el.dom.src || this.src;
    }
...
/*
	function(name, image_source, child_images) {
		console.log("Constructor called with: ", name, image_source);
		this.name = name;
...
Ext.define('Widget_Chooser', {
    ...
    display: function() {
        console.log("Widget Choose object. Variable = " + this.variable);
    },
...
console.log("Widget Chooser: load complete.");